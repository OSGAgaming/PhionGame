...
namespace QueefCord.Content.Entities
{
    public class Player : KinematicEntity2D, IUpdate, IDraw, IMappable
    {
        ...
        public Player()
        {
            AddMechanic(new PlayerMovement());
            ...
            AddMechanic(new RigidBody(0.01f));

            Get<EntityCollision>().AddCollisionBox(new Rectangle(20, 30, 8, 5));
        ...
        {
            Texture2D tex = Assets<Texture2D>.Get("Textures/Player/PlayerWalkAnimation");
            Color color = Color.Lerp(Color.IndianRed, Color.White, 1 - Get<BaseNPCStats>().ImmunityFrames / 30f);

            sb.Draw(tex, Transform.Position, Get<PlayerAnimation>().Frame, color, 1 - Center.Y / 100000f);
