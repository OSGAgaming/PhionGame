...
namespace QueefCord.Core.Helpers
{
   ...
   float x0, float x1, float x2)
        ...
        public static Vector2 ReturnIntersectRect(Vector2 p1, Vector2 p2, Rectangle r)
        {
            Vector2[] a =
            {
            ReturnIntersectionLine(p1, p2, new Vector2(r.X, r.Y), new Vector2(r.X + r.Width, r.Y)),
            ...
            Vector2 chosen = new Vector2(int.MaxValue);

            for (int i = 0; i < a.Length; i++)
            {
                if (a[i] != new Vector2(int.MaxValue))
                    if (Vector2.Distance(a[i], p1) < Vector2.Distance(chosen, p1) || chosen == new Vector2(int.MaxValue))
                        chosen = a[i];
        ...
        public static Vector2 ReturnIntersectionLine(Vector2 l1p1, Vector2 l1p2, Vector2 l2p1, Vector2 l2p2)
        {
            ...
            float d = (l1p2.X - l1p1.X) * (l2p2.Y - l2p1.Y) - (l1p2.Y - l1p1.Y) * (l2p2.X - l2p1.X);
            if (d == 0)
                return new Vector2(int.MaxValue);
            float r = q / d;
            q = (l1p1.Y - l2p1.Y) * (l1p2.X - l1p1.X) - (l1p1.X - l2p1.X) * (l1p2.Y - l1p1.Y);
            float s = q / d;
            if (r < 0 || r > 1 || s < 0 || s > 1)
        ...
        public static Vector2 GetClosestPoint(Rectangle[] world, Point p1, Point p2)
        {
            Vector2 chosen = new Vector2(int.MaxValue);

            foreach (Rectangle r in world)
            {
                if (LineIntersectsRect(p1, p2, r))
                {
                    Vector2 inter = ReturnIntersectRect(p1.ToVector2(), p2.ToVector2(), r);
                    if (chosen == new Vector2(int.MaxValue) || Vector2.Distance(inter, p1.ToVector2()) < Vector2.Distance(chosen, p1.ToVector2()))
                    {
                        chosen = inter;
                    }
                }
